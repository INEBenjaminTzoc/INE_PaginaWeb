...
import { Bar, BarChart, CartesianGrid, Label, Legend, Line, LineChart, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
    ...
    <>
      <div className='w-full px-15 lg:px-20 pb-10'>
        ...
        <p className="text-base mt-2 mb-4 text-slate-600">
          Análisis de datos de censos y encuestas poblacionales
        </p>
        <Carousel opts={{ align: "start", loop: true }} className="w-full">
          <CarouselContent>
            {censosEncuestas.map((chart) => (
              <CarouselItem key={chart.id} className="md:basis-1/2 lg:basis-1/3">
...
function ChartCard({ type, data }: ChartComponentProps) {
  ...
  const generateChartConfig = (data: any[]) => {
    return data.reduce((config, item, index) => {
      const color = COLORS[index % COLORS.length]; // Asigna un color cíclico
      item.fill = color; // Agrega el campo `fill` a cada elemento de la data
      config[item.name] = {
        label: item.name,
        color: color,
      };
